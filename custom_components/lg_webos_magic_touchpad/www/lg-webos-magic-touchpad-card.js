...
// src/lg-webos-magic-touchpad-card.ts
var LgWebosMagicTouchpadCard = class extends i4 {
  constructor() {
    super(...arguments);
  ...
  render() {
    ...
    return b2`
      <ha-card>
        ...
        ${this._config.show_nav_buttons ? b2`
              <div class="controls">
  ...
  _onTouchMove(event) {
    ...
    if (event.touches.length === 1 && this._lastPointer) {
      ...
      if (this._lastTouchDistance !== void 0) {
  ...
  async _onKeyDown(event) {
    if (event.key === "Backspace") {
      this._command("delete", void 0, false);
      return;
    }
    if (event.key === "Enter") {
      event.preventDefault();
...
LgWebosMagicTouchpadCard.properties = {
...
LgWebosMagicTouchpadCard.styles = i`
    :host {
      display: block;
